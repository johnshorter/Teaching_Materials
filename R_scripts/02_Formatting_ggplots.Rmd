...
Let's make the most basic plots first.
We can use some commands in base R to look at the data

## We can ask R to plot all the elements by entering the data set name with the plot function.

```{r}
plot(ToothGrowth) 
```

## We can make specific plots with telling it what to put on the Y and X axes by using the $. 
```{r}
plot(ToothGrowth$dose, ToothGrowth$len)
```

## We can make histograms to look at data distribution as well. Often a good place to data with a new data set.
```{r}
hist(ToothGrowth$len)
```


However, we can make better plots and figures!
...
And in the help menu under Cheat Sheets.

This module will go over different features of ggplot
...
Understanding the syntax of ggplot is the most important thing!
Dose on X, length on Y, color as supp for this dataset.
...
The line `ggplot(data = ...)` specifies the data file. It doesn't do much by itself. Try it if you don't believe me!

Then you add a new layer (in this case points) by using `+`and in a new line you write `geom_point()`.
"Geom" stands for geometric objects.
In this case `geom_points()` produces points.

The mapping argument is always paired with aesthetics  `aes()`, and the x and y arguments of `aes()` specify which variables to map to the x and y axes. ggplot2 looks for the mapped variables in the data argument.

Then to color by supp, you write `aes(.... , color = supp)`.

We can also save the plot and print it. Notice it now appears in your environment. 
```{r}
example1 = ggplot(data = ToothGrowth) +
  geom_point(mapping = aes(x = dose, y = len, color = supp)) 

example1
```

Here is another way to make the same plot! This is using the %>%. There are many different ways to get to the same outcome. 
```{r}
ToothGrowth %>% 
...
It's more readable this way. No need to specify data = blah and mapping = blah.
Thus `ToothGrowth %>% ggplot(...)`


These plots are not bad... Now let's use a custom palette called "wisteria".
...
You can customize color using `scale_color_manual()`, inside which, you specifies the names of the colors, in this case the color names are pre-specified by a vector called `wisteria`.
...
The `alpha = 0.8` makes your dots 20% transparent.
(`alpha = 1` is 0% transparent and default.) The `size` option allows you to make dots larger or smaller.
...
`shape = 21` inside `geom_point()` creates hollow dots.
`color = "black"` adds black borders to each dot.
...
Coming back to using open circles in `geom_point()`, Open circles takes both `fill` and `color`.
...
Now, to specify colors, I have to use `scale_fill_manual()`, instead of `scale_color_manual()`, because the variable `supp` is now reflected by fill, not color.

But there is still a problem... Note that when you use jitter, the horizontal spread is completely random.
...
ToothGrowth %>% 
  ...
  geom_point(aes(fill = supp), 
             position = position_jitter(0.1, seed = 666),   #added seed = 666 here. My favorite number. 
...
In this case, I used `seed = 666`. My favorite number. 
...
Then I put `fun = mean`, which asked it summarize y values using the `mean()` function.
Then I put `aes(group = supp, color = supp)`, which asked for one line per supp, and color those lines by supp.

Well, there's a problem.
The lines are not the same color as the dots.
That's because we specified the interior colors of dots using `scale_fill_manual()`, but we never specified the color of the lines.
It's easy to fix.
We will use `scale_color_manual()` to specify colors for the lines too.
...
ToothGrowth %>% 
  ...
  stat_summary(geom = "line",  
               ...
               aes(group = supp, color = supp),  
               size = 1.2) + 
  stat_summary(geom = "errorbar",  # draw error bars (another option is geom = "linerange")
               fun.data = mean_se, # mean_se specifies the error bar to be based on standard error (se)
...
Another common one is theme_bw(). Try it! what do you prefer?
Here are some others https://ggplot2.tidyverse.org/reference/ggtheme.html
...
The rule of thumb is if you want to adjust lines, such as axis line, then you use `element_line()` Then you specify color and size in `element_line()`.
...
The facet syntax works like this: `facet_grid(row ~ column)` We are using supp as columns, and only 1 row, so `facet_grid(. ~ supp)`.
The `.` before the `~` is just a space holder.
...
Note that this time the legend is redundant with the panel label, so I turn it off using `legend.position = "none"` in `theme()`.
Great!
...
guides() + 
theme_minimal() + 
theme(legend.position = ...) + theme(axis.line = element_line(size = 1.2)) + theme(text = element_text(size = 12, color = "black", face = "bold")) + theme(axis.text = element_text(size = 12, color = "black", face = "bold"))
